...
function LoginContent() {
  ...
  useEffect(() => {
    if (errorParam === 'admin_required') {
      ...
      setStatusMessage({
        type: 'error',
        text: 'Insufficient Permissions: Redirected to user workspace.',
      });
        ...
        <div className="bg-white/95 backdrop-blur-md rounded-3xl border-2 border-slate-300/80 shadow-2xl p-6 sm:p-8 space-y-6 tech-corner-box animate-tech-pulse relative overflow-hidden">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-xs font-mono text-slate-500">
              :: HARDWARE WORKSPACE AUTHENTICATION ::
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`p-3 rounded-xl border-2 text-left font-mono transition-all cursor-pointer flex items-center gap-2.5 ${
                  selectedRole === 'USER'
          ...
          <div className="space-y-4 pt-2">
            <button
              ...
              disabled={loading}
              className={`w-full py-4 px-5 rounded-2xl border-2 font-mono text-sm font-bold flex items-center justify-center gap-3 shadow-md hover:shadow-xl transition-all cursor-pointer group ${
                selectedRole === 'ADMIN'
                  ? 'border-slate-900 bg-slate-900 hover:bg-slate-800 text-white'
                  : 'border-slate-300 hover:border-[#0066FF] bg-white hover:bg-blue-50/50 text-slate-900'
              }`}
            >
